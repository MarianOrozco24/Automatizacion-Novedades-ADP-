...
database_omnia = os.getenv("DB_NAME_OMNIA")
database_atp = os.getenv("DB_NAME_ATP")
...
def conexion_db():
    try:
        cnx = mysql.connector.connect(host=host, user=user, password=password, database=database_omnia, use_pure=True)
...
def main(month,year):
    ...
    try:
        ...
        df_final.to_excel(f"{Config.ruta_excel}{file_name}", index=False)
        print(f"✅ Se extrajo correctamente el archivo {file_name} en {Config.ruta_excel}")
        mx.showinfo("Exportacion exitosa", f"Se exporto correctamente el archivo {file_name} en {Config.ruta_excel}")
...
# titulo
label_title = ctk.CTkLabel(ventana_principal, text="Administracion de Personal", font=("helvetica", 24, "bold"))
